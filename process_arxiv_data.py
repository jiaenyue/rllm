...
def write_sft_file(filepath, dataset, category_map, options_str):
    """Writes a dataset to an SFT JSONL file in the correct format."""
    count = 0
    instruction_text = "你是个优秀的论文分类师"
    with open(filepath, 'w', encoding='utf-8') as f:
        for entry in dataset:
            letter = category_map[entry["category"]]
            # Format matches data_sample/sftdata.jsonl
            input_text = (
                f"Based on the title '{entry['title']}', authors '{entry['authors']}', "
                f"and abstract '{entry['abstract']}', please determine the "
                f"scientific category of this paper.\n\n{options_str}"
            )
            sft_record = {
...
def process_arxiv_data(config):
    ...
    if generate_full_sft:
        ...
        full_count = write_sft_file(sft_full_filepath, sft_entries, category_to_letter, options_str)
        ...
        train_count = write_sft_file(sft_train_filepath, train_set, category_to_letter, options_str)
        val_count = write_sft_file(sft_val_filepath, val_set, category_to_letter, options_str)
